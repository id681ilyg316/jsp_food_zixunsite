...
<!-- servletformstart 
<form  action="/food_zixunsite/food_zixunsite?ac=guidestj&tglparentid=<%=tglparentid%>"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1"  onsubmit="return checkform()" >
  servletformend -->
  <!-- s2formstart -->
<form name="f1" onsubmit="return checkform()" action="/food_zixunsite/food_zixunsite/guidestj.do?tglparentid=<%=tglparentid%>" > 
  <!-- s2formend -->
<center>
...
if(gnoobj.value!=""){  
var ajax = new AJAX();
ajax.post(encodeURI("/food_zixunsite/factory/checkno.jsp?table=guides&col=gno&value="+gnoobj.value+"&checktype=insert&ttime=<%=Info.getDateStr()%>")) 
//ajax.post("/food_zixunsite/factory/checkno.jsp?table=guides&col=gno&value="+gnoobj.value+"&checktype=insert&ttime=<%=Info.getDateStr()%>") 
