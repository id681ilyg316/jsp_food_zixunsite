...
<!-- servletformstart 
<form  action="/food_zixunsite/food_zixunsite?ac=hysysuserxg&id=<%=mmm.get("id")%>"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1"  onsubmit="return checkform()" >
  servletformend -->
  <!-- s2formstart -->
<form  name="f1"  onsubmit="return checkform()" action="/food_zixunsite/food_zixunsite/hysysuserxg.do?id=<%=mmm.get("id")%>" > 
  <!-- s2formend -->
<center>
...
if(unameobj.value!=""){  
var ajax = new AJAX(); 
ajax.post("/food_zixunsite/factory/checkno.jsp?table=sysuser&col=uname&value="+unameobj.value+"&checktype=update&id=<%=mmm.get("id")%>&ttime=<%=Info.getDateStr()%>"); 
//ajax.post(encodeURI("/food_zixunsite/factory/checkno.jsp?table=sysuser&col=uname&value="+unameobj.value+"&checktype=update&id=<%=mmm.get("id")%>&ttime=<%=Info.getDateStr()%>")); 
