...
<!-- servletformstart 
<form  action="/food_zixunsite/food_zixunsite?ac=guidesxg&id=<%=mmm.get("id")%>"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1"  onsubmit="return checkform()" >
  servletformend -->
  <!-- s2formstart -->
<form  name="f1"  onsubmit="return checkform()" action="/food_zixunsite/food_zixunsite/guidesxg.do?id=<%=mmm.get("id")%>" > 
  <!-- s2formend -->
<center>
...
if(gnoobj.value!=""){  
var ajax = new AJAX(); 
ajax.post("/food_zixunsite/factory/checkno.jsp?table=guides&col=gno&value="+gnoobj.value+"&checktype=update&id=<%=mmm.get("id")%>&ttime=<%=Info.getDateStr()%>"); 
//ajax.post(encodeURI("/food_zixunsite/factory/checkno.jsp?table=guides&col=gno&value="+gnoobj.value+"&checktype=update&id=<%=mmm.get("id")%>&ttime=<%=Info.getDateStr()%>")); 
